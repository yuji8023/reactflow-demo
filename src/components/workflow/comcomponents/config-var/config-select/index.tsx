  ...
  return (
    <div>
      {options.length > 0 && (
        <div className="mb-1">
          <ReactSortable
            ...
            list={optionList}
            setList={(list: any) =>
              onChange(list.map((item: any) => item.name))
            }
            handle=".handle"
            ...
            {options.map((o, index) => (
              <div className={`${s.inputWrap} relative`} key={index}>
                ...
                <input
                  ...
                  className={
                    'h-9 w-full grow cursor-pointer border-0 bg-transparent pl-1.5 pr-8 text-sm leading-9 text-gray-900 focus:outline-none'
                  }
                />
                <DeleteOutlined
      ...
      >
        ...
        <div className="text-[13px] text-gray-500">添加选项</div>
      </div>
    </div>
  );
};
