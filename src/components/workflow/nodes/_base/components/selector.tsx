  ...
  return (
    ...
    >
      ...
      {showOption && !readonly && (
        <div
          className={cn(
            'absolute top-[24px] z-10 w-[120px]  select-none rounded-lg border border-gray-200 bg-white p-1 shadow-lg',
            popupClassName,
          )}
        >
          {list.map((item) => (
            ...
            >
              ...
              {showChecked && item.value === value && (
                <CheckSquareOutlined className="h-4 w-4 text-primary-600" />
              )}
            </div>
          ))}
        </div>
