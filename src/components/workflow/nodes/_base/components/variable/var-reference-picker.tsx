...
import type { CredentialFormSchema } from './types';
...
import { useIsChatMode, useWorkflowVariables } from '../../../../hooks';
...
const VarReferencePicker: FC<Props> = ({
  ...
  const outputVarNode = useMemo(() => {
    ...
    if (isLoopVar) return loopNode?.data;
  ...
  const VarPickerWrap = !isSupportConstantValue
    ? 'div'
    : PortalToFollowElemTrigger;
  return (
      ...
      >
        ...
        >
          <>
            {isAddBtnTrigger ? (
              <div>
                <Button icon={<PlusOutlined />} onClick={() => {}}></Button>
              </div>
            ) : (
              <div
                ...
                className={cn(
                  ...
                  !isSupportConstantValue &&
                    'rounded-lg bg-components-input-bg-normal p-1',
              ...
              >
                    ...
                    <TypeSelector
                      ...
                      trigger={
                        <div className="radius-md flex h-8 items-center bg-components-input-bg-normal px-2">
                          <div className="system-sm-regular mr-1 text-components-input-text-filled">
                            {
                              varKindTypes.find(
                                (item) => item.value === varKindType,
                              )?.label
                            }
                          </div>
                          <DownOutlined className="h-4 w-4 text-text-quaternary" />
                        </div>
                      }
                  ...
                  !hasValue && (
                    <div className="ml-1.5 mr-1">
                      <VariableX
                        className={`h-4 w-4 ${
                          readonly
                            ? 'text-components-input-text-disabled'
                  ...
                  >
                    <div
                      ...
                      className={cn(
                        'h-full',
                        isSupportConstantValue &&
                          'flex items-center rounded-lg bg-gray-100 py-1 pl-1',
                      )}
                    >
                      <Tooltip
                        popupContent={!isValidVar && hasValue && '无效的变量'}
                      >
                        <div
                          className={cn(
                            'h-full items-center rounded-[5px] px-1.5',
                            hasValue ? 'inline-flex bg-white' : 'flex',
                          )}
                        >
                          {hasValue ? (
                            <>
                              ...
                              <div className="flex items-center text-primary-600">
                                {!hasValue && (
                                  <VariableX className="h-3.5 w-3.5" />
                                )}
                                {/* {isEnv && (
                                  <Env className="h-3.5 w-3.5 text-util-colors-violet-violet-600" />
                                )} */}
                                {/* {isChatVar && (
                                ...
                                <div
                                  className={cn(
                                    ...
                                    isChatVar &&
                                      'text-util-colors-teal-teal-700',
                              ...
                              {!isValidVar && (
                                <ExclamationCircleFilled className="ml-0.5 h-3 w-3 text-[#D92D20]" />
                              )}
                            </>
                          ) : (
                            <div
                              className={`overflow-hidden ${
                                readonly
                                  ? 'text-components-input-text-disabled'
                                  : 'text-components-input-text-placeholder'
                              } system-sm-regular text-ellipsis`}
                            >
                              {placeholder ?? '设置变量值'}
                            </div>
                          )}
                ...
                {hasValue && !readonly && !isInTable && (
                  ...
                  >
                    <div className="h-3.5 w-3.5 text-center text-gray-500 group-hover:text-gray-800">
                      <CloseOutlined
                        style={{ verticalAlign: 'super', width: '10px' }}
                      />
                    </div>
                  </div>
                )}
                {!hasValue && valueTypePlaceHolder && (
                  <Tag className=" absolute right-1 top-[50%] translate-y-[-50%] capitalize">
                    {valueTypePlaceHolder}
                  </Tag>
                )}
              </div>
            ...
            {!readonly && isInTable && (
              <RemoveButton
                className="absolute right-1 top-0.5 hidden h-5 group-hover/picker-trigger-wrap:block"
                onClick={() => onRemove?.()}
              />
            )}

            {!hasValue && typePlaceHolder && (
              <Tag className="absolute right-2 top-1.5">{typePlaceHolder}</Tag>
            )}
          </>
        </WrapElem>
        <PortalToFollowElemContent
