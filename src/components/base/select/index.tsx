    ...
    >
      <div className={classNames('relative')}>
        <div className="group text-text-secondary">
          {allowSearch ? (
            <ComboboxInput
              className={`w-full rounded-lg border-0 ${bgClassName} py-1.5 pl-3 pr-10 shadow-sm focus-visible:bg-state-base-hover focus-visible:outline-none group-hover:bg-state-base-hover sm:text-sm sm:leading-6 ${
            ...
            <ComboboxButton
              ...
              className={classNames(
                `flex h-9 w-full items-center rounded-lg border-0 ${bgClassName} py-1.5 pl-3 pr-10 shadow-sm focus-visible:bg-state-base-hover focus-visible:outline-none group-hover:bg-state-base-hover sm:text-sm sm:leading-6`,
                optionClassName,
              )}
            >
              <div
        ...
        {filteredItems.length > 0 && open && (
          ...
          >
            {filteredItems.map((item: Item) => (
              ...
              >
                {({ /* active, */ selected }: { selected: boolean }) => (
                  <>
                    {renderOption ? (
    ...
    >
      ...
      >
        ...
        {!renderTrigger && (
          <ListboxButton
            className={classNames(
              `flex h-full w-full items-center rounded-lg border-0 bg-components-input-bg-normal pl-3 pr-10 focus-visible:bg-state-base-hover-alt focus-visible:outline-none group-hover/simple-select:bg-state-base-hover-alt sm:text-sm sm:leading-6 ${
                disabled ? 'cursor-not-allowed' : 'cursor-pointer'
              }`,
              className,
            )}
        ...
        {!disabled && (
          <ListboxOptions
            className={classNames(
              'absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border-[0.5px] border-components-panel-border bg-components-panel-bg-blur px-1 py-1 text-base shadow-lg backdrop-blur-sm focus:outline-none sm:text-sm',
              optionWrapClassName,
            )}
          >
            {items.map((item: Item) => (
      ...
      >
        {renderTrigger ? (
          ...
          >
            ...
            <DownOutlined className="h-4 w-4 shrink-0 text-text-quaternary group-hover:text-text-secondary" />
          </div>
        )}
      </PortalToFollowElemTrigger>
            ...
            >
              <span className="w-0 grow truncate" title={item.name}>
                ...
                {item.value === installedValue && (
                  <Tag className="ml-1 shrink-0">INSTALLED</Tag>
                )}
              </span>
              {!hideChecked && item.value === value && (
                <CheckOutlined className="h-4 w-4 shrink-0 text-text-accent" />
              )}
            </div>
          ))}
        </div>
